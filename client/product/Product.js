...
export default function Product ({match}) {
    ...
    const imageUrl = product._id ? `/api/product/image/${product._id}?${new Date().getTime()}` : '/api/product/defaultphoto'
    ...
    return (
        <div className={classes.root}>
            <Grid container spacing={10}>
                        ...
                        <div className={classes.flex}>
                            ...
                            <Typography component="p" variant="subtitle1" className={classes.subheading} >
                                ...
                                <Link to={'/shops/'+product.shop._id} className={classes.link} >
                                    <span>
                                        <Icon className={classes.icon}>shoppig_basket</Icon>{product.shop.name}
                                    </span>
                                </Link>
                            </Typography>
                        </div>
                ...
                {suggestions.length > 0 && (<Grid item xs={5} sm={5} ><Suggestions products={suggestions} title="Related Products" /></Grid>)}
            </Grid>
        </div>
    )
}