...
const PlaceOrder = (props) => {
    ...
    const placeOrder = () => {
        props.stripe.creteToken().then(payload => {
            if(payload.error) {
    ...
    return (
        <span>
            ...
            <CardElement className={classes.StripeElement} {...{style: {
                base: {
                    color: '#424770',
            ...
            <div className={classes.checkout}>
                {values.error && (<Typography component="span" color="error" className={classes.error}><Icon color="error" className={classes.errorIcon} >error</Icon>{values.error}</Typography>)
            }
